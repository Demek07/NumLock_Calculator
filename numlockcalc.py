...
class MiniCalcWindow(QWidget):
    ...
    def _safe_eval(self, expression: str) -> tuple:
        ...
        # Заменяем "X%" → "*X/100" только если перед % есть оператор или (
        # Но для "A-B%" нужно учитывать A
        # Поэтому:
        # 1. Находим все вхождения "число%"
        # 2. Для каждого — ищем ближайшее число *до* "%", которое и будет основой
        # Но проще: если выражение содержит %, пробуем парсить его структуру

        # 🔑 Используем regex + замена вручную — для надёжности
        # Шаг 1: Ищем шаблоны "X%$", "X%)" — это "% от X"
        # Шаг 2: Ищем "A op X%" — это "% от A"

        # Попробуем: заменить "%", стоящие в конце или после оператора, на "*X/100*A", где A — первое число
        # Но это сложно без парсера.

        # ✅ Проще: заменим "X%" на "*X/100", а затем в "A op X%" — заменим "X%*A" на "X/100*A", но это не сработает.

        # ✅ Решение через AST (надёжно)
        # НО: eval — безопаснее не использовать AST, просто используем регулярки.

        # 🔧 Вариант: "A op B%" → "A * (1 op_sign * B/100)"
        # Но для "A+B%C" это не сработает.

        # 🔧 Вариант: "X%" всегда → "*(X/100)*last_operand", где last_operand — число перед оператором или началом.

        # 🎯 Реализуем "ручной eval" с поддержкой процентов:
        # 1. Разбиваем на токены
        # 2. Заменяем X% → [X, "%"] и обрабатываем "%", как бинарный оператор с левой ассоциативностью и специальным поведением.

        # Но это много кода.

        # ✅ Альтернатива: заменим "%", стоящий после оператора или скобки, на "* (X/100) * Y", где Y — результат предыдущего выражения до него.

        # 🧠 Самое простое — **предварительно обработать выражение**, заменяя все "число%" на "*(число/100)*", и добавить вспомогательную переменную prev.

        # Но проще всего:
        # 1. Если выражение содержит только `%`, и `%` в конце или после оператора — парсим по паттерну.

        # 🔧 Поддержка для простых случаев:
        #   - "X%" → X/100
        #   - "A op X%" → A op (X/100 * A)
        #   - "A op B op2 X%" → (A op B) op2 (X/100 * (A op B))

        # Реализуем:
        # Шаг 1: разбиваем на части по операторам, но с сохранением порядка.

        # ✅ Простое, но рабочее решение:
        # Заменим все "число%" → "* (число/100) * prev", но prev нужно отслеживать.
        # Для этого — создадим "обёртку" через exec() + локальные переменные.

        # Но безопасность важна → используем eval с обёрткой.

        # 🟡 Финальное решение:
        # 1. Проверим, есть ли "%"
        # 2. Если есть — заменим "%", стоящие после оператора (или в конце), на "*X/100", где X — число до "%"
        # 3. Но для "A+B%C" нужно знать, что C% относится к (A+B)

        # ✅ Сделаем так:
        # Если выражение содержит "%", и оно соответствует шаблону:
        #   ^(\d+(?:\.\d+)?)$%$ → X%
        #   ^(\d+(?:\.\d+)?)([\+\-\*\/])(\d+)%$ → A op B%
        #   ^(.+)([\+\-\*\/])(\d+)%$ → expr op B%
        ...
        #   "X%" → X/100
        #   "A op B%" → A op (B/100 * A)
        #   "expr op B%" → eval(expr) op (B/100 * eval(expr))

        # Реализуем:

        # 🔍 Разбираем структуру с помощью regex
        # Паттерн: `expr op number%`
        pattern = r'^(.+?)([\+\-\*\/])(\d+(?:\.\d+)?)%$'
